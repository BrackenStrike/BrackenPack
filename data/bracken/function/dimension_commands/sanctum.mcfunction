...
# Dimension Travel
execute if score @s bp.1_second matches 4 if data storage bracken:config {dimension_travel: true} if entity @s[y=320,dy=100] run function bracken:dimension_crossing/sanctum_to_nether
...
execute if data storage bracken:config {dimension_travel: true} run effect give @s[predicate=!bracken:sneak,y=289,dy=100] levitation 1 2 true
