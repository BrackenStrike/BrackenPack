...
execute if score @s bp.kraken matches ..0 run function bracken:entity/boss/kraken/misc/attack_selector
scoreboard players remove @s bp.kraken 1

#spin
scoreboard players remove @s[scores={bp.spin=1..}] bp.spin 1
execute at @s[scores={bp.spin=1..}] run function bracken:entity/boss/kraken/misc/spin
...
execute if entity @s[scores={bp.sploosh=20..}] run summon area_effect_cloud ^ ^4 ^-12 {custom_particle:{type:"squid_ink"},CustomNameVisible:0b,Radius:1f,RadiusPerTick:0.02f,RadiusOnUse:0.1f,Duration:300,potion_contents:{custom_effects:[{id:"minecraft:wither",amplifier:0,duration:200,show_particles:1b,show_icon:1b},{id:"minecraft:blindness",amplifier:0,duration:200,show_particles:1b,show_icon:1b}]},CustomName:"Ink Bomb"}
...
#death
execute if entity @s[scores={bp.kraken.health=..20}] run function bracken:entity/boss/kraken/misc/death
