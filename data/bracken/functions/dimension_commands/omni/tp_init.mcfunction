...
$execute if entity @a[scores={bp.omni_id=$(target_id)}] run tellraw @s[tag=!bp.omnitp_queued] [{"text":"- ","font":"alt","obfuscated":true,"color":"dark_green","bold":true},{"text":"ID LOCATED, TP IN PROGRESS","color":"green","obfuscated":"false","font":"default"},{"text":"- ","font":"alt","obfuscated":true,"color":"dark_green","bold":true}]
$execute if entity @a[scores={bp.omni_id=$(target_id)}] run tag @s add bp.omnitp_queued
